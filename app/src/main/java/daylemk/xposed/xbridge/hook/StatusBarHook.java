...
/**
 * Status bar hook Created by DayLemK on 2015/4/30.
 */
public class StatusBarHook extends Hook {
	...
	// set the field and we can use it later to collapse notification or unlock
	// screen
	...
	public void initZygote(StartupParam startupParam) throws Throwable {
		...
		// inspectItemButton =
		// sModuleRes.fwd(R.layout.notification_inspect_item);
	}
	...
	public void handleLoadPackage(
		...
		Class<?> systemUIAppClass = XposedHelpers.findClass(
				StaticData.PKG_NAME_SYSTEMUI + ".SystemUIApplication",
				loadPackageParam.classLoader);
		XposedBridge.hookAllMethods(systemUIAppClass, "onCreate",
				new XC_MethodHook() {
					@Override
					protected void afterHookedMethod(MethodHookParam param)
						...
						Log.d(TAG, "app: " + app);
						app.registerReceiver(new OnPreferenceChangedReceiver(),
								new IntentFilter(
										StaticData.ACTION_PREFERENCE_CHANGED));
					}
				});

		final Class<?> baseStatusBarClass = XposedHelpers.findClass(
				StaticData.PKG_NAME_SYSTEMUI + ".statusbar.BaseStatusBar",
				loadPackageParam.classLoader);
		Log.d(TAG, "BaseStatusBar: " + baseStatusBarClass);
		XposedBridge.hookAllMethods(baseStatusBarClass, "inflateGuts",
				new XC_MethodHook() {
					@Override
					protected void afterHookedMethod(MethodHookParam param)
							throws Throwable {
						...
						if (!param.thisObject.equals(statusBarObject)) {
							Log.w(TAG, "statusBar is different: "
									+ statusBarObject);
						...
						if (idGuts == -1) {
							...
							idGuts = res.getIdentifier("notification_guts",
									"id", StaticData.PKG_NAME_SYSTEMUI);
						}
						Log.d(TAG, "guts id: " + idGuts);

						// move get guts view here, so we can check if the
						// action is already injected or not
						final Object expandNotiRowObject = param.args[0];
						Log.d(TAG, "expandNotiRowObject: "
								+ expandNotiRowObject);
						final FrameLayout layoutGuts = (FrameLayout) XposedHelpers
								.callMethod(expandNotiRowObject,
                                        "findViewById", idGuts);
						Log.d(TAG, "get the guts view: " + layoutGuts);
						Log.d(TAG,
								"guts children number: "
										+ layoutGuts.getChildCount());
						// check if the view already exists
						// EDIT: we should use the dynamic generated id, 'cause
						// that will not be equal
						...
						if (PlayAction.isShow && PlayAction.isShowInStatusBar) {
							isPlayNeed2Add = Action.isNeed2Add(layoutGuts,
									PlayAction.class);
						}
						if (AppOpsAction.isShow
								&& AppOpsAction.isShowInStatusBar) {
							isOpsNeed2Add = Action.isNeed2Add(layoutGuts,
									AppOpsAction.class);
						}
						if (AppSettingsAction.isShow
								&& AppSettingsAction.isShowInStatusBar) {
							isAppSetNeed2Add = Action.isNeed2Add(layoutGuts,
									AppSettingsAction.class);
						}
						if (ClipBoardAction.isShow
								&& ClipBoardAction.isShowInStatusBar) {
							isClipBoardNeed2Add = Action.isNeed2Add(layoutGuts,
									ClipBoardAction.class);
						}
						if (SearchAction.isShow
								&& SearchAction.isShowInStatusBar) {
							isSearchNeed2Add = Action.isNeed2Add(layoutGuts,
									SearchAction.class);
						}
						if (XPrivacyAction.isShow
								&& XPrivacyAction.isShowInStatusBar) {
							isXPrivacyNeed2Add = Action.isNeed2Add(layoutGuts,
									XPrivacyAction.class);
						}
						if (AppInfoAction.isShow
								&& AppInfoAction.isShowInStatusBar) {
							isAppInfoNeed2Add = Action.isNeed2Add(layoutGuts,
									AppInfoAction.class);
						}
						if (NotifyCleanAction.isShow
								&& NotifyCleanAction.isShowInStatusBar) {
							isNotifyCleanNeed2Add = Action.isNeed2Add(
									layoutGuts, NotifyCleanAction.class);
						}

						if (!(isPlayNeed2Add || isOpsNeed2Add
								|| isAppSetNeed2Add || isClipBoardNeed2Add
								|| isSearchNeed2Add || isXPrivacyNeed2Add
								|| isAppInfoNeed2Add || isNotifyCleanNeed2Add)) {
							Log.d(TAG, "need add nothing");
							return;
						}
						// here, we do some prepare for the button general
						// stuff, like get package name,
						...
						Object statusBarNotificationObject = XposedHelpers
								.callMethod(expandNotiRowObject,
                                        "getStatusBarNotification");
						Log.d(TAG, "statusBarNotificationObject: "
								+ statusBarNotificationObject);
						final String pkgName = (String) XposedHelpers
								.callMethod(statusBarNotificationObject,
                                        "getPackageName");
						Log.i(TAG, "package name: " + pkgName);

						// 4) init the XBridge button
						// the first child is a linearLayout in the aosp
						final LinearLayout linearLayout = (LinearLayout) layoutGuts
								.getChildAt(0);
						 ...
						 * XposedHelpers.callMethod(XposedHelpers.callMethod
						 * (statusBarNotificationObject, "getUser"),
						 * "getIdentifier"); Log.d(TAG, "user id: " + userId);
						 * packageManager = (PackageManager)
						 * XposedHelpers.callMethod (statusBarObject,
						 * "getPackageManagerForUser", userId); Log.d(TAG,
						 * "packageManager: " + packageManager);
						 */
	...
	private void getInspectLayoutParams(Resources res, FrameLayout layoutGuts) {
		if (inspectLayoutParams == null) {
			final int idInspectItemId = res.getIdentifier(
					"notification_inspect_item", "id",
					StaticData.PKG_NAME_SYSTEMUI);
			Log.d(TAG, "inspect item id: " + idInspectItemId);

			// 2) get the guts view and inspect item button view
			final ImageButton imageButtonInspect = (ImageButton) layoutGuts
					.findViewById(idInspectItemId);
			Log.d(TAG, "the inspect image button: " + imageButtonInspect);
			inspectLayoutParams = new LinearLayout.LayoutParams(
					imageButtonInspect.getLayoutParams());
			...
			Log.d(TAG, "image button inspect: "
					+ StatusBarHook.this.inspectLayoutParams.toString());
		}
	}

	private void addViewAndSetAction(Action action, LinearLayout linearLayout,
			String pkgName) {
		ImageButton xBridgeButton = createXBridgeButton(context,
				inspectLayoutParams);
		action.setAction(StatusBarHook.this, context, pkgName, xBridgeButton);
		// add the view to the last-1
		// EDIT: move to last one, 'cause some notification has more than one
		// icon
		linearLayout.addView(xBridgeButton, linearLayout.getChildCount());
	}

	private ImageButton createXBridgeButton(Context context,
		...
		ImageButton xBridgeButton = (ImageButton) LayoutInflater.from(
				xBridgeContext).inflate(R.layout.notification_inspect_item,
                null);
		...
		xBridgeButton.setPadding(xBridgeButton.getPaddingLeft() / 2,
				xBridgeButton.getPaddingTop() / 2,
				xBridgeButton.getPaddingRight() / 2,
				xBridgeButton.getPaddingBottom() / 2);

		return xBridgeButton;
	}

	public void dismissKeyguardAndStartAction(
			final OnDismissKeyguardAction onDismissKeyguardAction) {
		...
		// try {
		// method =
		// statusBarObject.getClass().getSuperclass().getDeclaredMethod(
		...
		Object mStatusBarKeyguardViewManager = XposedHelpers.getObjectField(
                statusBarObject, "mStatusBarKeyguardViewManager");
		Log.d(TAG, "get the keyguardViewManager: "
				+ mStatusBarKeyguardViewManager);
		final boolean keyguardShowing = (boolean) XposedHelpers.callMethod(
				mStatusBarKeyguardViewManager, "isShowing");
		Log.d(TAG, "keyguardShowing: " + keyguardShowing);

		Object onDismissAction = Proxy.newProxyInstance(
				onDismissActionInterface.getClassLoader(),
				new Class<?>[] { onDismissActionInterface },
					...
					public Object invoke(Object o, Method method,
						...
						if (method.getName().equals("onDismiss")) {
							AsyncTask.execute(new Runnable() {
								@Override
								public void run() {
									Log.i(TAG, "keyguardShowing: "
											+ keyguardShowing);
									if (keyguardShowing) {
										Class<?> activityManagerNativeClass = XposedHelpers
												.findClass(
										...
										if (activityManagerNativeClass != null) {
											Object defaultNative = XposedHelpers
													.callStaticMethod(
                                                            activityManagerNativeClass,
                                                            "getDefault");
											Log.d(TAG, "defaultNative: "
													+ defaultNative);
											// call
											// keyguardWaitingForActivityDrawn
											// method
											XposedHelpers
													.callMethod(defaultNative,
                                                            "keyguardWaitingForActivityDrawn");
										}
									}
							...
							XposedHelpers.callMethod(statusBarObject,
									"animateCollapsePanels", FLAG_EXCLUDE_NONE,
									true /* force */);
							return true;
		...
		XposedHelpers.callMethod(statusBarObject, "dismissKeyguardThenExecute",
				onDismissAction, false/* afterKeyguardGone */);
	}
