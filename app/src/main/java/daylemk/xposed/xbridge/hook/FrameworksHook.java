...
/**
 * Created by DayLemK on 2015/5/11. the frameworks hook to get some drawable etc
 * from frameworks
 */
public class FrameworksHook extends Hook {
	...
	public static void getDefaultIcons(Resources sModRes) {
		...
		if (iconCopyDefault == null) {
			iconCopyDefault = sModRes
					.getDrawable(R.drawable.ic_menu_copy_material);
		}
		if (iconSearchDefault == null) {
			iconSearchDefault = sModRes
					.getDrawable(R.drawable.ic_menu_search_material);
		}
		Log.d(TAG, "default icons: " + iconCopyDefault + ", "
				+ iconSearchDefault);
	}

	private static int getCopyId(Resources sModRes) {
		return sModRes.getIdentifier("ic_menu_copy_material", "drawable",
				StaticData.PKG_NAME_FRAMEWORK);
	}

	private static int getSearchId(Resources sModRes) {
		return sModRes.getIdentifier("ic_menu_search_material", "drawable",
				StaticData.PKG_NAME_FRAMEWORK);
	}
	...
	public void handleInitPackageResources(
		...
		if (iconCopy == null) {
			...
			// use null theme
			iconCopy = initPackageResourcesParam.res.getDrawable(
					getCopyId(initPackageResourcesParam.res), null);
		}
		if (iconSearch == null) {
			Log.d(TAG, "iconSearch is null");
			iconSearch = initPackageResourcesParam.res.getDrawable(
					getSearchId(initPackageResourcesParam.res), null);
		}
