...
/**
 * App info screen hook Created by DayLemK on 2015/4/28.
 */
public class AppInfoHook extends Hook {
	...
	public void handleLoadPackage(
		...
		final Class<?> installedAppDetailsClass = XposedHelpers.findClass(
				StaticData.PKG_NAME_SETTINGS + ".applications"
						+ ".InstalledAppDetails", loadPackageParam.classLoader);
		Log.d(TAG, "installedAppDetailsClass: " + installedAppDetailsClass);
		XposedHelpers.findAndHookMethod(installedAppDetailsClass,
				"onCreateOptionsMenu",
				// add parameters type here
				Menu.class, MenuInflater.class, new XC_MethodHook() {
					@Override
					protected void afterHookedMethod(MethodHookParam param)
						...
						PackageManager pm = (PackageManager) XposedHelpers
								.getObjectField(installedAppDetails, "mPm");
						final PackageInfo pkgInfo = (PackageInfo) XposedHelpers
								.getObjectField(installedAppDetails,
                                        "mPackageInfo");
						final Activity activity = (Activity) XposedHelpers
								.callMethod(installedAppDetails, "getActivity");
	...
	private void handleAllAction(Menu menu, Context context, String pkgName) {
		...
		/*
		 * if(AppOpsAction.isShowInAppInfo) { final Action action = new
		 * AppOpsAction(); MenuItem playMenuItem =
		 * menu.add(action.getMenuTitle()); action.setAction(AppInfoHook.this,
		 * context, pkgName, playMenuItem); }
		 */
	...
	private void addMenuAndSetAction(Menu menu, Action action, Context context,
			String pkgName) {
