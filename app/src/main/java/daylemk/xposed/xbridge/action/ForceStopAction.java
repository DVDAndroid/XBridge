...
/**
 * Created by DayLemK on 2015/5/21. Force stop a package, require SU permission
 * through handleData method AND include dismiss button mod
 */
public class ForceStopAction extends Action {
	...
	public static void loadPreferenceKeys(Resources sModRes) {
		...
		showDismissButtonDefault = sModRes
				.getBoolean(R.bool.show_dismiss_default);
	}
	...
	public static void loadPreference(SharedPreferences preferences) {
		...
		Log.d(TAG, "load preference: " + "isShow:" + isShow
				+ "isShowDismissButton:" + isShowDismissButton);
	}
	...
	public static boolean onReceiveNewValue(String key, String value) {
		...
		if (key.equals(keyShow)) {
			...
			/*
			 * } else if (key.equals(keyShowInRecentTask)) { isShowInRecentTask
			 * = Boolean.valueOf(value); } else if
			 * (key.equals(keyShowInStatusBar)) { isShowInStatusBar =
