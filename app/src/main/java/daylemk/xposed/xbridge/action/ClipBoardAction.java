...
public class ClipBoardAction extends Action {
	...
	public static void loadPreferenceKeys(Resources sModRes) {
		...
		keyShowInRecentTask = sModRes
				.getString(R.string.key_clipboard_recent_task);
		keyShowInStatusBar = sModRes
				.getString(R.string.key_clipboard_status_bar);
		// get the default value of this action
		showInStatusBarDefault = sModRes
				.getBoolean(R.bool.clipboard_status_bar_default);
		showInRecentTaskDefault = sModRes
				.getBoolean(R.bool.clipboard_recent_task_default);
		showInAppInfoDefault = sModRes
				.getBoolean(R.bool.clipboard_app_info_default);
	...
	public void handleData(final Context context, final String pkgName) {
		...
		new Handler(context.getMainLooper()).post(new Runnable() {
			...
			public void run() {
				...
				ClipData clipData = ClipData.newPlainText(ARG_PACKAGE_NAME,
						pkgName);
				clipboardManager.setPrimaryClip(clipData);
				Context xBridgeContext = Hook.getXBridgeContext(context);

				String str = xBridgeContext
						.getString(R.string.package_name_copied);
				Toast.makeText(context, str + "\n" + pkgName, Toast.LENGTH_LONG)
						.show();
			}
		});
	}
