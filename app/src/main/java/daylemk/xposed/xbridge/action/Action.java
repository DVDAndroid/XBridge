...
public abstract class Action implements StatusBarHook.OnDismissKeyguardAction {
	...
	// not use this, 'cause we need read value at the begin of injection. so,
	// the static boolean
	// value is faster.
	...
	public static boolean onReceiveNewValue(String key, String value) {
		if (key != null && value != null) {
			if (!(PlayAction.onReceiveNewValue(key, value)
					...
					|| NotifyCleanAction.onReceiveNewValue(key, value) || SearchAction
						.onReceiveNewValue(key, value))) {
	...
	public static boolean isActionsShowInAppInfo() {
		return PlayAction.isShow
				&& PlayAction.isShowInAppInfo
				// || AppOpsAction.isShow && AppOpsAction.isShowInAppInfo
				|| AppSettingsAction.isShow
				&& AppSettingsAction.isShowInAppInfo || SearchAction.isShow
				&& SearchAction.isShowInAppInfo || XPrivacyAction.isShow
				&& XPrivacyAction.isShowInAppInfo || NotifyCleanAction.isShow
				&& NotifyCleanAction.isShowInAppInfo
	...
	public static boolean isActionsShowInRecentTask() {
		return PlayAction.isShow && PlayAction.isShowInRecentTask
				...
				|| AppSettingsAction.isShow
				&& AppSettingsAction.isShowInRecentTask || SearchAction.isShow
				&& SearchAction.isShowInRecentTask || XPrivacyAction.isShow
				&& XPrivacyAction.isShowInRecentTask || AppInfoAction.isShow
				&& AppInfoAction.isShowInRecentTask || NotifyCleanAction.isShow
				&& NotifyCleanAction.isShowInRecentTask
	...
	public static boolean isActionsShowInStatusBar() {
		return PlayAction.isShow && PlayAction.isShowInStatusBar
				...
				|| AppSettingsAction.isShow
				&& AppSettingsAction.isShowInStatusBar || SearchAction.isShow
				&& SearchAction.isShowInStatusBar || XPrivacyAction.isShow
				&& XPrivacyAction.isShowInStatusBar || AppInfoAction.isShow
				&& AppInfoAction.isShowInStatusBar || NotifyCleanAction.isShow
	...
	/**
	 * get the view id of this action, we need make it public, so we can get the
	 * previous view from layout
	 */
	...
	protected Drawable getPackageIcon(PackageManager packageManager,
			String pkgName) {
		Drawable drawable;
		try {
			...
			Log.w(TAG, "package: " + pkgName
					+ ", icon can't be found, use generic icon");
	...
	/**
	 * subclass should overwrite this method or handleData method this method
	 * will be called first, if the result is null, will call handleData method <br>
	 * EDIT: add Context parameter <br>
	 * EDIT: this method ONLY is used for start activity, not for broadcast. For
	 * broadcast, handle in the handleData method
	 */
	protected abstract Intent getIntent(Hook hook, Context context,
			String pkgName);

	/**
	 * subclass should overwrite this method or getIntent method <br>
	 * EDIT: this class is need to set public to call out of the action <br>
	 * EDIT: If you like to send a boardcast, do in the this method, not
	 * getIntent.
	 */
	...
	public void setAction(final Hook hook, final Context context,
			final String pkgName, View view) {
		mContext = context;
		mPkgName = pkgName;
		...
		view.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View view) {
				Log.d(TAG, "on click action is: " + Action.this.toString()
						+ ", hook: " + hook);
				...
				if (hook instanceof StatusBarHook) {
					...
					((StatusBarHook) hook)
							.dismissKeyguardAndStartAction(Action.this);
				} else {
	...
	public void setAction(final Hook hook, final Context context,
			final String pkgName, MenuItem menuItem) {
		mContext = context;
		mPkgName = pkgName;
		// set the show flag
		...
		menuItem.setOnMenuItemClickListener(new MenuItem.OnMenuItemClickListener() {
			...
			public boolean onMenuItemClick(MenuItem item) {
				Log.d(TAG, "on click action is: " + Action.this.getClass()
						+ ", hook: " + hook);
				mIntent = getFinalIntent(hook, context, pkgName);
				if (hook instanceof AppInfoHook && mIntent != null) {
					try {
						// no need to start as user, 'cause when the appInfo
						// screen is called, it
						...
						XBridgeToast.showToast(
								context,
								Hook.getXBridgeContext(context).getString(
										R.string.error)
	...
	public void startIntentAsUser(final Context context, final Intent intent,
			int appUid) {
		...
		try {
			...
			// if the target package is no available, this will throw a
			// exception
			...
			// show toast
			XBridgeToast.showToastOnHandler(context,
					Hook.getXBridgeContext(context).getString(R.string.error)
				...
				new Thread(new Runnable() {
					...
					public void run() {
						...
						try {
							...
							e1.printStackTrace();
							XposedBridge.log(e);
							List<PackageInfo> infoList = context
									.getPackageManager()
		...
		int userId = (int) XposedHelpers.callStaticMethod(UserHandle.class,
				"getUserId", appUid);
